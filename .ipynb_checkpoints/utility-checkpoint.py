...
def get_eeg_features(raw_eeg):
    ...
    all_features = []
    for i in range(0, len(segments), 32):
        batch = torch.tensor(segments[i:i+32], dtype=torch.float32).to(device)
        with torch.no_grad():
            features = model.forward_features(batch)
            features = features.mean(dim=0).cpu()
        all_features.append(features)
    final_features = torch.stack(all_features).mean(dim=0)        
...
def get_selected_eeg_features(raw_eeg):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
def get_selected_eeg_features_from_path(eeg_path):
    raw = mne.io.read_raw_eeglab(eeg_path, preload=True)
    return get_selected_eeg_features(raw)

def save_selected_eeg_features_from_path(eeg_path, store_path, identification):
    eeg_features = get_selected_eeg_features_from_path(eeg_path)
    save_path = store_path + f"/small_features_{identification}.pt"
    torch.save(eeg_features, save_path)
    return save_path
